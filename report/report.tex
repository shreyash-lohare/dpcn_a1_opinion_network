...
Team \textbf{Edge Runners}. All code, figures, and the data pipeline are
...
All Person 2 analyses run on the same $91$-respondent, ordinal-imputed matrix
described in Section~\ref{sec:imputation}, so no part of this report silently
uses a different sample.

\textbf{Findings.} Correlations estimated from $n=91$ respondents scatter about
zero with standard deviation $1/\sqrt{n-3}=0.107$ even when nothing is related.
The permuted null reproduces this closely at $\mathbf{0.106}$, against
$\mathbf{0.139}$ for the observed matrix. Real structure exists, but it is
...
\textbf{Implication.} At the conventional cutoff $|r|>0.15$ well over half the
...
columns; observed SD $0.139$ against $0.106$ for the null, with the excess
...
$|r|>0.15$ more than half of surviving edges are chance.}
\label{fig:g4}
\end{figure*}
...
\textbf{Findings.} With $T=91$ and $N=60$, $Q=T/N=1.517$ and
$\lambda_+=(1+1/\sqrt{Q})^2=\mathbf{3.283}$. Figure~\ref{fig:g5} shows the bulk
of the spectrum tracking the theoretical density closely, with
\textbf{exactly four} eigenvalues clearing the band---$4.38$, $4.06$, $3.55$ and
$3.35$---together carrying $\mathbf{25.6\%}$ of total variance. The count is
comfortable rather than marginal: the fourth mode clears the edge by $2.0\%$
while the fifth, at $2.86$, sits well inside the noise band.

Reading the item loadings, the four modes correspond to (i)~environmental and
social commitment against technology governance and assessment tradition,
...
technological action against individual and community obligation. Modes (i)
and (iii) split cleanly along topic blocks; (ii) and (iv) cut across them, and
we offer those two as readings of the loadings rather than as established
constructs.

\textbf{Implication.} Three quarters of this correlation matrix is sampling
noise. That is the single most important quantitative statement in the analysis,
and it disciplines everything downstream: the network is rebuilt from the four
...
Section~\ref{sec:discussion}: four modest, largely cross-cutting dimensions
carrying roughly a quarter of variance is the signature of a broadly agreeing cohort
with local structure, not of a polarised one.
...
($T=91$, $N=60$, $Q=1.517$). The bulk tracks the theoretical curve; exactly
four eigenvalues exceed $\lambda_+=3.283$, carrying $25.6\%$ of total
variance. Inset: the top six eigenvalues, showing a clear gap between the fourth
($3.35$) and the fifth ($2.86$).}
\label{fig:g5}
\end{figure*}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{../figures/person3/graph_6_reordered_heatmap.pdf}
\caption{Item--item correlations (MP-denoised), reordered by Louvain community. The 15 communities are separated by black lines, with topic blocks marked in the margins. The communities cut across the predefined topic blocks, highlighting the divergence between the survey's structural design and respondents' actual association patterns.}
\caption{Item--item correlations (MP-denoised), reordered by Leiden community. The 15 communities are separated by black lines, with topic blocks marked in the margins. The communities cut across the predefined topic blocks, highlighting the divergence between the survey's structural design and respondents' actual association patterns.}
\label{fig:g6}
\end{figure}
...
tracking edge count, giant-component fraction, fragmentation and Leiden
...
remains above $4$}, so that community and path-based metrics stay
meaningful---and note that spectral denoising has already removed the sampling
noise the chance column measures, so raising the cutoff further would penalise
the same noise twice. This selects $\boldsymbol{|r|>0.22}$, giving
$\mathbf{131}$ edges ($68$ positive, $63$ negative), mean degree $4.37$, a giant
component of $50$ of $60$ items ($83\%$), and modularity $Q=0.472$. At the next
step up, $|r|>0.23$, mean degree falls to $3.47$ and the rule stops being
satisfied.
...
edges, leaving roughly $138$ real ones; spectral denoising, which never sees the
null, retains $131$. The estimates differ by about $5\%$.

\textbf{Implication.} Panel~(d) is the cautionary one. Modularity climbs
steadily to $0.72$ as the threshold rises---but at that point the graph is $43$
...
chosen cutoff. (c) Components and isolates. (d) Leiden modularity, which rises
to $0.72$ only once the graph has broken into $43$ pieces. The vertical line
marks the selected $|r|>0.22$.}
\label{fig:g7}
\end{figure*}
...
Figure~\ref{fig:g10} compares the observed community structure of the item network at $|r| > 0.22$ against three null models: a degree-preserving rewiring \cite{maslov2002specificity}, an Erd\H{o}s--R\'enyi graph, and a column-permutation null that scrambles responses independently per item.
Figure~\ref{fig:g10} compares the observed community structure of the item network at $|r| > 0.22$ against three null models: a degree-preserving rewiring \cite{maslov2002specificity}, an Erd\H{o}s--R\'enyi graph, and a column-permutation null that scrambles responses independently per item. We also compare three community detection algorithms to demonstrate their methodological differences.

\textbf{Findings and Implication.} The Louvain algorithm detects 15 communities, yielding a modularity of $Q=0.472$. This value decisively clears the Erd\H{o}s--R\'enyi null ($Q=0.407\pm0.017$, $z=3.91$, $p=0.005$) and the rewiring null ($Q=0.326\pm0.014$, $z=10.39$, $p=0.005$). However, it fails to exceed the column-permutation null ($Q=0.624\pm0.046$, $z=-3.32$, $p=1.000$). This corroborates the respondent-network results (Section~\ref{sec:nulls-p2}): the modularity is an artefact of thresholding a correlation matrix derived from small-$N$ data, not an emergent property of the opinion network itself.
\textbf{Findings and Implication.} The Leiden algorithm detects 15 communities, yielding a modularity of $Q=0.491$ (improving upon Louvain's $Q=0.472$). This value decisively clears the Erd\H{o}s--R\'enyi null ($Q=0.420\pm0.016$, $z=4.46$, $p=0.005$) and the rewiring null ($Q=0.333\pm0.013$, $z=12.13$, $p=0.005$). However, it fails to exceed the column-permutation null ($Q=0.629\pm0.044$, $z=-3.14$, $p=1.000$). This corroborates the respondent-network results (Section~\ref{sec:nulls-p2}): the modularity is an artefact of thresholding a correlation matrix derived from small-$N$ data, not an emergent property of the opinion network itself.
\textbf{Louvain vs. Leiden Community Detection.} Initially, we applied the widely used Louvain algorithm, which detected 15 communities with a modularity of $Q=0.472$. However, Louvain suffers from a well-documented flaw: its greedy aggregation phase can yield internally disconnected or poorly connected communities, artificially capping the quality of the partition \cite{traag2019louvain}. To address this, we subsequently applied the Leiden algorithm, which introduces a refinement phase to guarantee well-connected communities. Leiden successfully found a superior partition on our item network, increasing the modularity to $Q=0.491$ while still resolving into 15 communities. We therefore use the Leiden partition for our primary structural analysis.
\textbf{Algorithm comparison.} All three algorithms partition the 60-item network, but they use fundamentally different strategies:

\textbf{Findings against Null Models.} The Leiden modularity ($Q=0.491$) decisively clears the Erd\H{o}s--R\'enyi null ($Q=0.420\pm0.016$, $z=4.46$, $p=0.005$) and the degree-preserving rewiring null ($Q=0.333\pm0.013$, $z=12.13$, $p=0.005$). However, it fails to exceed the column-permutation null ($Q=0.629\pm0.044$, $z=-3.14$, $p=1.000$). This corroborates the respondent-network results (Section~\ref{sec:nulls-p2}): the modularity is an artefact of thresholding a correlation matrix derived from small-$N$ data, not an emergent property of the opinion network itself.
\begin{itemize}
    \item \textbf{Louvain} \cite{blondel2008fast}: An agglomerative method that greedily merges nodes to maximise modularity. Fast ($O(n \log n)$), but its merge phase can produce poorly-connected communities. Achieved $Q=0.472$, $k=15$.
    \item \textbf{Leiden} \cite{traag2019louvain}: Extends Louvain with a \emph{refinement phase} that breaks merged super-nodes back apart to guarantee well-connected communities. Found a strictly superior partition: $Q=0.491$, $k=15$.
    \item \textbf{Girvan-Newman} \cite{newman2006modularity}: A \emph{divisive} method that works top-down by iteratively removing the edge with the highest betweenness centrality, splitting the network progressively. The $Q(k)$ curve (Figure~\ref{fig:g10alg}b) peaks at $k=15$, $Q=0.447$, then monotonically declines, confirming 15 as the natural scale of community structure. GN is the only method that reveals how modularity evolves across all partition sizes.
\end{itemize}

Leiden achieves the highest peak modularity, confirming it as the best choice for the primary analysis. The consistent agreement of all three algorithms on $k=15$ communities reinforces this as a robust structural feature of the network.

\textbf{Findings against null models (Leiden, Q=0.491).} This value decisively clears the Erd\H{o}s--R\'enyi null ($Q=0.420\pm0.016$, $z=4.46$, $p=0.005$) and the degree-preserving rewiring null ($Q=0.333\pm0.013$, $z=12.13$, $p=0.005$). However, it fails to exceed the column-permutation null ($Q=0.629\pm0.044$, $z=-3.14$, $p=1.000$). This corroborates the respondent-network results (Section~\ref{sec:nulls-p2}): the modularity is an artefact of thresholding a correlation matrix derived from small-$N$ data, not an emergent property of the opinion network itself.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../figures/person3/graph_10_algorithm_comparison.pdf}
\caption{Three-algorithm comparison on the item network. (a) Peak modularity $Q$ for each algorithm; all converge on $k=15$ communities, with Leiden finding the highest $Q=0.491$. (b) The Girvan-Newman $Q(k)$ curve showing how modularity rises to a clear peak at $k=15$ before declining; horizontal lines mark Louvain and Leiden peak values for reference. (c) Rank-ordered community size distributions; all three algorithms produce similarly distributed community sizes, confirming the partition is structurally stable across methods.}
\label{fig:g10alg}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../figures/person3/graph_10_communities_nulls.pdf}
\caption{(a) The item network thresholded at $|r|>0.22$, nodes coloured by Louvain community (borders indicate the predefined thematic block). (b) Observed modularity $Q=0.472$ sits comfortably above the Erd\H{o}s--R\'enyi and degree-preserving rewiring nulls, but fails to exceed the rigorous column-permutation null, mirroring the topological artefact found in the respondent network.}
\caption{(a) The item network thresholded at $|r|>0.22$, nodes coloured by Leiden community (borders indicate the predefined thematic block). (b) Observed modularity $Q=0.491$ sits comfortably above the Erd\H{o}s--R\'enyi and degree-preserving rewiring nulls, but fails to exceed the rigorous column-permutation null, mirroring the topological artefact found in the respondent network.}
\caption{(a) The item network thresholded at $|r|>0.22$, nodes coloured by Leiden community (borders indicate the predefined thematic block). (b) Observed Leiden modularity $Q=0.491$ sits comfortably above the Erd\H{o}s--R\'enyi and degree-preserving rewiring nulls, but fails to exceed the rigorous column-permutation null, mirroring the topological artefact found in the respondent network.}
\label{fig:g10}
\end{figure*}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{../figures/person3/graph_11_structural_balance.pdf}
...
exactly four eigenvalue modes exceed the Marchenko--Pastur band, carrying
$25.6\%$ of total variance. The remaining $74.4\%$ is what $91$ respondents
...
network ($91$ nodes, $298$ edges, one component, symmetrised $k$NN at $k=4$),
observed modularity is $0.421$ and average clustering $0.219$:
...
\midrule
Degree-preserving rewiring & $0.337$ & $0.011$ & $7.46$ \\
Erd\H{o}s--R\'enyi $G(n,m)$ & $0.337$ & $0.011$ & $7.28$ \\
Column permutation & $0.404$ & $0.018$ & $\mathbf{0.91}$ \\
\bottomrule
\end{tabular}
\end{center}
...
approaching $7.5$. Against the null that reproduces the construction it vanishes
($p=0.19$). We report this as a finding rather than a failure: those communities
...
Four largely cross-cutting modes carrying roughly a quarter of variance,
communities that fail their own null, and $79.8\%$ agreement across every block
describe a cohort in broad consensus with local, domain-specific disagreement.
They do not describe two camps.

\paragraph{Communities cut across thematic boundaries.}
Despite the clear block-diagonal structure of the item network when ordered by community (Graph 6), these data-driven groupings do not align with the four predefined survey themes (Technology, Education, Society/Ethics, Environment). With an NMI of just $0.201$, the structure reveals that associations are formed across distinct themes rather than strictly within them.
Despite the clear block-diagonal structure of the item network when ordered by community (Graph 6), these data-driven groupings do not align with the four predefined survey themes (Technology, Education, Society/Ethics, Environment). With an NMI of just $0.205$, the structure reveals that associations are formed across distinct themes rather than strictly within them.

\paragraph{Structural balance strengthens with noise filtering.}
The hypothesis of structural balance is robustly confirmed in Graph 11. As the correlation threshold is tightened from $|r|=0.05$ up to $|r|=0.22$, the fraction of balanced triads monotonically approaches $100\%$, while the sign-permutation null stays flat at $50\%$. The increasing balance indicates that the network encodes genuine, self-consistent opinion structures that crystallise as random noise is removed.
...
component, while on the respondent network it implies $|r|>0.506$ and leaves
$410$ edges across $22$ components with $20$ isolates.

\textbf{Raw uncentred similarity} leaves $624$ of $1{,}770$ pairs above
$|r|>0.22$ --- a $35\%$-dense graph in which $83\%$ of all correlations are
positive. This is the same acquiescence effect quantified in
Section~\ref{sec:g3}, restated at the threshold actually used.

\textbf{Simple column fills instead of the ordinal model} were tested rather
than assumed. Filling the same $242$ cells in the same $91$ respondents with the
column mean gives correlation SD $0.135$ and $185$ edges at $|r|>0.22$; the
column median gives $0.136$ and $184$; the adopted ordinal-regression imputation
gives $0.139$ and $205$. Critically, \emph{all three recover exactly four
significant eigenvalue modes}, carrying $24.7\%$, $25.0\%$ and $25.6\%$ of
variance respectively. The dimensional structure is therefore a property of the
data rather than of the imputer, which is the strongest available defence of the
modelling choice: it is not doing the work that the conclusions rest on.

\textbf{Dropping the ten respondents with more than twelve missing items},
leaving $86$ and filling the remaining $47$ cells with a column statistic, is
the one alternative that does change a headline. At $T=86$, $Q=1.433$ and
$\lambda_+=3.368$, and only \emph{three} modes clear the band, carrying
$19.3\%$ of variance. We report this rather than bury it: the count of
significant dimensions is robust to how missing cells are filled but not to
which respondents are retained. Keeping all $91$ non-empty respondents is the
better-supported choice --- discarding usable respondents to avoid modelling
$4.4\%$ of cells trades more information than it protects --- but the reader
should know the result is conditional on it.
...
columns puts the noise floor at $0.106$ against an observed correlation spread
of $0.139$, and Marchenko--Pastur filtering leaves exactly four modes above the
band, carrying $25.6\%$ of variance. The edge threshold was then selected from a
$46$-point sweep by a stated rule rather than convention, at $|r|>0.22$, where
the giant component still spans $83\%$ of the items.
...
null at $z=7.46$ and an Erd\H{o}s--R\'enyi null at $z=7.28$, but fails the
permutation null at $z=0.91$ ($p=0.19$): it is an artefact of correlating and
...
with genuine but local structure --- four modest, largely cross-cutting
dimensions, not two camps.

Finally, the detected item-network communities do not align cleanly with the predefined thematic blocks (NMI = $0.201$), indicating that respondent opinions are formed across thematic boundaries. However, these opinions exhibit a remarkable internal consistency: as noise edges are removed, structural balance in the network steadily approaches $100\%$, establishing a robust, self-consistent topology despite the absence of distinct ideological camps.
Finally, the detected item-network communities do not align cleanly with the predefined thematic blocks (NMI = $0.205$), indicating that respondent opinions are formed across thematic boundaries. However, these opinions exhibit a remarkable internal consistency: as noise edges are removed, structural balance in the network steadily approaches $100\%$, establishing a robust, self-consistent topology despite the absence of distinct ideological camps.
...
\addlinespace
Dev Patel & Developed \texttt{src/person3} modules and exploration notebooks. Conducted community analysis and null model comparisons on the item network (Graph 10). Performed structural balance percolation sweep (Graph 11) and constructed the reordered MP-denoised correlation heatmap (Graph 6). Drafted corresponding report sections (Graphs 6, 10, 11, Discussion, and Conclusion). Commits tagged \texttt{[P3]} or related to Graph 6/10/11. \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
