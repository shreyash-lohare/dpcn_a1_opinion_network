...
that Person 2 owns (src/person2/, which regenerates the whole
...
Inputs are Person 1 artefacts only (the imputed and row-centred matrices from
src/pipeline.py), so this graph is independent of every downstream handoff.
"""
...
from src.loader import parse_question_columns
...
def plot_block_connectivity(
    output_path: Path = FIGURES_DIR / "graph9_topic_block_connectivity.png",
