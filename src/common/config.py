"""Shared constants for the Person 2 analysis layer.

Every module imports seed, paths, and palette from here rather than redefining
them. Kept separate from ``src/config.py`` (Person 1's ``PipelineConfig``, which
configures the sanitisation/imputation stage) so the two owners do not edit the
same file.

The palette is not invented here: it is the one already used by Person 1's
committed figures 1-3 (``src/diagnostics.py``), adopted verbatim so that no
...
# Neutral accents reused across the Person 2 figures.
REAL_COLOR = "#1B998B"
NULL_COLOR = "#E4572E"
MARK_COLOR = "#2B3A55"

# --- frozen pipeline constants (CLAUDE.md section 2) -----------------------
...
# --- figure output ---------------------------------------------------------
DPI = 200  # matches Person 1's committed figures


def ensure_dirs() -> None:
    """Create output directories if absent.

    Deliberately a function rather than import-time code: CLAUDE.md section 5
    ...
    for person in ("person1", "person2", "person3"):
        (FIGDIR / person).mkdir(parents=True, exist_ok=True)
